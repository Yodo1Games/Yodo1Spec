...
#if DEBUG
#define OpenSuitPayStoreLog(...) NSLog(@"[ OpenSuit ]: %@", [NSString stringWithFormat:__VA_ARGS__]);
#else
#define OpenSuitPayStoreLog(...)
#endif

@protocol OpenSuitPayStoreContentDownloader;
@protocol OpenSuitPayStoreReceiptVerificator;
@protocol OpenSuitPayStoreTransactionPersistor;
@protocol OpenSuitPayStoreObserver;

extern NSString *const OpenSuitPayStoreErrorDomain;
extern NSInteger const OpenSuitPayStoreErrorCodeDownloadCanceled;
extern NSInteger const OpenSuitPayStoreErrorCodeUnknownProductIdentifier;
extern NSInteger const OpenSuitPayStoreErrorCodeUnableToCompleteVerification;

/** A StoreKit wrapper that adds blocks and notifications, plus optional receipt verification and purchase management.
 */
@interface OpenSuitPayStore : NSObject<SKPaymentTransactionObserver>
 ...
 */
@property (nonatomic, weak) id<OpenSuitPayStoreContentDownloader> contentDownloader;

/** The receipt verificator. You can provide your own or use one of the reference implementations provided by the library.
 @see OpenSuitPayStoreAppReceiptVerificator
 @see OpenSuitPayStoreTransactionReceiptVerificator
 */
@property (nonatomic, weak) id<OpenSuitPayStoreReceiptVerificator> receiptVerificator;
...
/**
 ...
 @see OpenSuitPayStoreKeychainPersistence
 @see OpenSuitPayStoreUserDefaultsPersistence
 */
@property (nonatomic, weak) id<OpenSuitPayStoreTransactionPersistor> transactionPersistor;
...
@protocol OpenSuitPayStoreContentDownloader <NSObject>
...
/**
 ...
 @discussion Hosted content from Apple’s server (@c SKDownload) is handled automatically by OpenSuitPayStore.
 */
...
@protocol OpenSuitPayStoreTransactionPersistor<NSObject>

- (void)persistTransaction:(SKPaymentTransaction*)transaction;

@end

@protocol OpenSuitPayStoreReceiptVerificator <NSObject>
...
/** Verifies the given transaction and calls the given success or failure block accordingly.
 ...
 @param failureBlock Called if the transaction failed verification. If verification could not be completed (e.g., due to connection issues), then error must be of code OpenSuitPayStoreErrorCodeUnableToCompleteVerification to prevent OpenSuitPayStore to finish the transaction. Must be called in the main queu.
 */
...
@protocol OpenSuitPayStoreObserver<NSObject>
@optional
 ...
 */
@interface NSNotification(OpenSuitPayStore)
