...
#ifndef OpenSuitPayment_h
#define OpenSuitPayment_h
...
@interface PayUser : NSObject <NSSecureCoding>
/// 玩家Id
...
@interface PayProductInfo : NSObject
/// 产品ID
...
@interface PaySubscriptionInfo : NSObject
///通用产品id
...
@interface OpenSuitPayment:NSObject

@property(nonatomic,strong)NSString* regionCode;
@property(nonatomic,strong)PayProductInfo* itemInfo;
 ...
 */
- (void)verifyAppStoreIAPOrder:(PayProductInfo *)itemInfo
                      callback:(void (^)(BOOL verifySuccess,NSString* response,NSError* error))callback;

/**
 * 查询订阅
 */
- (void)querySubscriptions:(PayProductInfo *)itemInfo
 ...
 *  上报订单已支付成功接口
 */
- (void)clientCallback:(PayProductInfo *)itemInfo callbakc:(void (^)(BOOL success,NSString* error))callback;

/**
 *  上报支付失败接口
 */
- (void)reportOrderStatus:(PayProductInfo *)itemInfo callbakc:(void (^)(BOOL success,NSString* error))callback;
